...
def _call_ollama(prompt: str, *, format: str | None = None, timeout: int = 15) -> str:
...
def _call_api(prompt: str, *, format: str | None = None, timeout: int = 15) -> str:
    if not API_BASE_URL:
...
def call_model(
    ...
    *,
    ...
    if MODEL_PROVIDER == "api":
        return _call_api(prompt, format=format, timeout=timeout)

    return _call_ollama(prompt, format=format, timeout=timeout)
